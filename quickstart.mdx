...
Before you start, make sure you have your API key. If you haven't set it up yet, see [Authentication](/authentication) to get one from your Organization Admin.

<Steps>
  <Step title="Set your credentials">
    Visit the **Settings page ([https://app.raptorcomply.com/settings](https://app.raptorcomply.com/settings))** in Raptor Comply. Note: only Organization Admins can access this page. Export your API key as an environment variable. This keeps your credentials out of your code and command history.

    ```bash theme={null}
    export RAPTOR_COMPLY_API_KEY=rc_live_xxxxxxxxxxxxxxxxxxxxxxxx
    ```

    Replace the value with your actual API key. The key identifies your organization on its own, so no slug is needed. Every example in this guide reads from this variable.
  </Step>
