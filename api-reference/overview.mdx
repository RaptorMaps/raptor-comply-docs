---
title: "Raptor Comply REST API Overview and Reference Guide"
description: "Access NERC OT compliance data with the Raptor Comply REST API. All requests use HTTPS, return JSON, and authenticate with a single X-API-Key header."
---

> Access NERC OT compliance data with the Raptor Comply REST API. All requests use HTTPS, return JSON, and authenticate with a single X-API-Key header.
...
The key is scoped to one organization and identifies it on every request, so no slug or second header is needed. The API rejects any request that is missing the header. For step-by-step instructions on generating a key, see [Authentication](/authentication).
...
| Physical Areas   | Full CRUD                       | Available   |
...
<CardGroup cols={2}>
  ...
  <Card title="Physical Areas" icon="door-open" href="/api-reference/physical-areas">
    Manage PSPs and low-impact areas, and the cyber assets they contain.
  </Card>
