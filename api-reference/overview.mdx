...
description: "Access NERC OT compliance data with the Raptor Comply REST API. All requests use HTTPS, return JSON, and require X-API-Key and Organization headers."
---

> Access NERC OT compliance data with the Raptor Comply REST API. All requests use HTTPS, return JSON, and require X-API-Key and Organization headers.
...
| `Organization` | Your organization slug, visible in your Raptor Comply tenant URL      |

The API rejects any request that is missing either header. For step-by-step instructions on generating a key and finding your organization slug, see [Authentication](/authentication).
