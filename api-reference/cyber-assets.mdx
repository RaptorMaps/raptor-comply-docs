...
<Note>
  Every request to the Raptor Comply API requires two headers: **X-API-Key** (your API key) and **Organization** (your organization slug). See [Authentication](/authentication) for setup instructions.
</Note>
...
curl --request GET \
  ...
  --header 'Organization: your-org-slug'
```
...
<ResponseField name="organization_slug" type="string">
  The slug of the organization that owns this cyber asset. Matches the value you pass in the `Organization` header.
</ResponseField>
...
<ResponseField name="organization_slug" type="string">
  The organization slug derived from your `Organization` header.
</ResponseField>
