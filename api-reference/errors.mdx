...
Your request was malformed: the JSON body could not be parsed, or a required element was missing and could not be defaulted.
...
The resource you requested does not exist, or it belongs to a different organization than the one your API key is scoped to. Double-check the ID in the path and confirm it belongs to your tenant.
...
The three authentication-specific failures each have an exact, stable JSON body you can match against the `error` and `message` fields in your error-handling logic:
